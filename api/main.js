...
module.exports = async (request, response) => {
  ...
  try {
    ...
    if (action === 'testSheetConnection') {
      const doc = await getDoc();
      const userSheet = doc.sheetsByTitle['users'];
      if (!userSheet) throw new Error("Sheet 'users' not found.");
      return response.status(200).json({ status: 'success', message: `Successfully connected to sheet '${doc.title}' and found sheet 'users' with ${userSheet.rowCount} rows.` });
    }
